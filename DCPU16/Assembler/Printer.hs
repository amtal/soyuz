...
pprint = render . vcat . map pI

pI :: Instruction -> Doc
pI (Comment xs) = semi <> text xs
pI (Label s) = colon <> text (unpack s)
pI (Data x) = nest 16 $ text "dat" <+> pW x
pI (Basic op a b) = nest 16 $ pBO op <+> pO a <> comma <+> pO b
pI (NonBasic op a) = nest 16 $ pNBO op <+> pO a
